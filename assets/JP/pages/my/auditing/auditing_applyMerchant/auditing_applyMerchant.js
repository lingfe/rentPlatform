...
Page({
  ...
  //根据状态获取商户申请数据
  getWhereState:function(state){
    var that=this;
    wx.request({
      url: app.config.zberPath_web + 'zber_sys/user_apply_merchant/getWhereState',
      method:"get",
      data:{
        state:state,
      },
  ...
  tabClick: function (e) {
    ...
    if (name == "未审核"){
      //未审核
      that.getWhereState(0);
    }else if(name == "通过"){
      //通过
      that.getWhereState(1);
    } else if (name == "不通过"){
      //不通过
      that.getWhereState(2);
    }
  ...
  bindtapnostate:function(e){
    var that = this;
    wx.request({
      url: app.config.zberPath_web + 'zber_sys/user_apply_merchant/updateState',
      method: "Post",
      header: {
      ...
      success: function (res) {
        ...
        //根据状态获取商户申请数据
        that.getWhereState(that.data.state);
      }
    });
  },
